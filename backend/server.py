...
# ============== PRE-ORDER RESERVATIONS (Next-day meal head-count) ==============
# Employees reserve their meals for the NEXT day. Vendor uses head-count to plan prep.
# No payment — reservations are a count-only mechanism.

MEAL_PERIODS = ["breakfast", "lunch", "snacks", "dinner"]
DEFAULT_RESERVATION_SETTINGS: Dict[str, Any] = {
    "breakfast": {"enabled": True, "cutoff_hour": 20, "cutoff_minute": 0},  # 8 PM previous day
...
class ReservationCreate(BaseModel):
    ...
    delivery_date: Optional[str] = None  # ISO YYYY-MM-DD. If omitted, defaults to tomorrow.
...
class ReservationSettingsUpdate(BaseModel):
    ...
    cutoff_hour: Optional[int] = None  # 0-23. Applied uniformly to all meals.
...
def _get_site_reservation_settings(site_doc: Dict[str, Any]) -> Dict[str, Any]:
    """Returns the per-meal settings for a site, with defaults filled in."""
...
def _cutoff_for_delivery_date(delivery_date: datetime, meal_cfg: Dict[str, Any]) -> datetime:
    """Cutoff is 'cutoff_hour' on the day BEFORE delivery_date (in IST)."""
    day_before = delivery_date - timedelta(days=1)
    # Build naive IST datetime at cutoff hour, then store as UTC-aware
    cutoff_ist = datetime(
        day_before.year, day_before.month, day_before.day,
        meal_cfg.get("cutoff_hour", 20), meal_cfg.get("cutoff_minute", 0),
        tzinfo=timezone.utc,
    )
    # Subtract IST offset so the stored UTC time corresponds to that IST clock time
    return cutoff_ist - timedelta(hours=5, minutes=30)


def _parse_delivery_date(date_str: Optional[str]) -> datetime:
    """Returns tomorrow's date (IST) if not provided. Always returns a date at 00:00 IST as UTC-aware datetime."""
    if not date_str:
    ...
    # Return a UTC-aware datetime representing midnight IST of that date
    return datetime(tomorrow.year, tomorrow.month, tomorrow.day, 0, 0, tzinfo=timezone.utc) - timedelta(hours=5, minutes=30)


@api_router.get("/reservations/availability")
async def get_reservation_availability(user: dict = Depends(get_current_user)):
    """Returns the 4 meal slots for TOMORROW with reservation status:
       enabled, cutoff_at, already_reserved, eligible_vendors."""
    if user["role"] != "employee":
    ...
    delivery_date = _parse_delivery_date(None)  # tomorrow
    ...
    # Existing reservations of this employee for tomorrow (to disable double-reserve)
    ...
    for r in existing:
        existing_by_meal[r["meal_period"]] = {
            "id": str(r["_id"]),
            "vendor_id": r.get("vendor_id"),
            "vendor_name": next((v["name"] for v in vendors_list if v["id"] == r.get("vendor_id")), ""),
        }
...
async def create_reservation(data: ReservationCreate, user: dict = Depends(get_current_user)):
    """Create a next-day reservation. Validates cutoff + one-per-meal-per-day rule."""
    if user["role"] != "employee":
        raise HTTPException(status_code=403, detail="Only employees can reserve meals")
    ...
    # Validate delivery date: must be tomorrow (anything else is rejected)
    ...
    # Validate vendor belongs to this site
    ...
    # Generate a pickup QR token (mirror existing /orders QR pattern)
    ...
    doc = {
        ...
        "delivery_date": delivery_date,
        "cutoff_at": cutoff_at,
        "status": "reserved",  # reserved | consumed | cancelled | no_show
    ...
    # Notify vendor + employee (push + in-app, best effort)
    try:
        await create_notification(
            user_id=user["id"],
...
async def list_my_reservations(
    status: Optional[str] = Query(None, description="Filter: reserved | consumed | cancelled | no_show | all"),
    user: dict = Depends(get_current_user),
):
    if user["role"] != "employee":
...
async def cancel_reservation(reservation_id: str, user: dict = Depends(get_current_user)):
    """Cancel a reservation. Allowed only before its cutoff."""
    ...
    # MongoDB returns naive UTC datetimes; coerce to aware for comparison
...
@api_router.post("/reservations/{reservation_id}/consume")
async def consume_reservation(reservation_id: str, user: dict = Depends(get_current_user)):
    """Vendor marks a reservation as consumed (after QR scan)."""
...
async def vendor_reservation_counts(
    ...
    # List of reservations for prep planning
...
@api_router.get("/reservations/admin/summary")
async def admin_reservation_summary(
    date: Optional[str] = Query(None, description="YYYY-MM-DD; defaults to tomorrow"),
    user: dict = Depends(get_current_user),
):
    """Site/master admin: aggregated counts by site and vendor for a date."""
    ...
    rows = []
    async for r in db.reservations.aggregate(pipeline):
        rows.append({
            "site_id": r["_id"]["site_id"],
            "vendor_id": r["_id"]["vendor_id"],
            "meal_period": r["_id"]["meal_period"],
            "reserved": r["count"],
            "consumed": r["consumed"],
        })
    total_by_meal = {m: 0 for m in MEAL_PERIODS}
    for r in rows:
        total_by_meal[r["meal_period"]] = total_by_meal.get(r["meal_period"], 0) + r["reserved"]
    return {
        "date": delivery_date.date().isoformat(),
...
@api_router.get("/sites/{site_id}/reservation-settings")
async def get_reservation_settings(site_id: str, user: dict = Depends(get_current_user)):
    """Get reservation toggle + cutoff config for a site."""
    if user["role"] not in ("master_admin", "site_admin", "super_admin", "city_admin"):
        raise HTTPException(status_code=403, detail="Not authorised")
...
@api_router.patch("/sites/{site_id}/reservation-settings")
async def update_reservation_settings(
    site_id: str,
    data: ReservationSettingsUpdate,
    user: dict = Depends(get_current_user),
):
    """Toggle per-meal reservations on/off + change cutoff hour. Master + Site Admin only."""
