...
**Frontend**: Navbar `OutletSwitcher` dropdown (only for operators w/ ≥2 outlets) → switch + reload; Vendor `Dashboard` "All outlets · today" overview with active highlight; **`vendor/AllOrders.js` "All Outlets" combined live-order feed** (route `/vendor/all-orders`, nav link shown only for operators, per-outlet colour tags + filter chips, auto-refresh 15s, `GET /vendor/all-outlets-orders`); `master/Admins.js` new role "Vendor Operator (multi-outlet)" (disabled password, outlet multi-select, operator cards w/ resend + delete, copy-link modal). Also fixed Admins.js to stringify FastAPI 422 detail arrays (was crashing React on invalid email).
